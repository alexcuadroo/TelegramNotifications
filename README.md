...
TelegramNotifications es un plugin para servidores PaperMC que permite enviar notificaciones a un chat de Telegram cuando ocurren ciertos eventos en el servidor de Minecraft.
...
## Instalación
1. Descarga el archivo JAR compilado de TelegramNotifications.
2. Coloca el archivo en la carpeta `plugins` de tu servidor PaperMC.
3. Inicia el servidor para que se genere el archivo de configuración `config.yml` en la carpeta `plugins/TelegramNotifications`.
...
## Comandos
- `/telereload` — Recarga la configuración del plugin sin reiniciar el servidor. Permisos: telegramnotifications.reload, por defecto solo los OP lo tienen activo.
...
## Permisos
- `telegramnotifications.reload` — Permite usar el comando de recarga.
